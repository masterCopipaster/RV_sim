...
int main(int argc, char** argv) {
	...
	rewind(pyfile);
	char* pybytes = (char*)calloc(pyfile_size + 1, sizeof(char));
	fread((void*)pybytes, sizeof(char), pyfile_size + 10, pyfile);
	...
	for (uint16_t i = 0; i < elf_hdr.phnum; i++) {
		size_t offset = elf_hdr.phoff + i * elf_hdr.phentsize;
		Elf32_Phdr phdr;
		...
		printf("\n");
#endif
		uint32_t* segment_content = (uint32_t*)(pybytes + phdr.off);
		uint32_t addr_aligned = phdr.vaddr;//(phdr.p_vaddr & ((phdr.p_align & phdr.p_align == 0) - 1)) + phdr.p_align; 
		for (int wordn = 0; wordn < phdr.memsz / 4; wordn++)
		{
